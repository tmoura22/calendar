...
class ICalServiceTest {
    ...
    void render_emptyMeetings_returnsBasicCalendar() {
        User owner = new User("tiago", "tiago@example.com", "pass");
        String result = iCalService.render(owner, List.of());

        assertThat(result).contains("BEGIN:VCALENDAR");
        assertThat(result).contains("X-WR-CALNAME:tiago's meetings");
    ...
    void render_withMeetings_rendersEventsCorrectly() {
        User organizer = new User("tiago", "tiago@test.com", "pass");
        User invitee = new User("moura", "moura@test.com", "pass");
        ...
        assertThat(result).contains("ORGANIZER;CN=tiago:mailto:tiago@test.com");
        assertThat(result).contains("ATTENDEE;CN=moura;PARTSTAT=NEEDS-ACTION:mailto:moura@test.com");
